...
**Pattern 7 — Split-button primary actions** (`09`). The email compose view's "Send email" button has an attached dropdown chevron for related options, rather than a separate second button crammed alongside it. **Apply to:** the Export action — when Figma export is enabled, **"Export to Figma" is the main button** (product heart) with an attached chevron for ZIP / Notion / related options; when Figma is gated off for store builds, the primary falls back to "Export as ZIP". Never bury Figma behind ZIP while the feature is on.
