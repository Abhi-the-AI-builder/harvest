...
Acopio ("the extension") is a Chrome extension for design research. This policy describes what data the extension handles and how.
...
When you collect items, Acopio stores them in **IndexedDB** on your device, inside the extension's own storage. This includes:
...
Acopio only makes network requests when you are using a feature that requires it:
...
| Sign in with Google | OAuth tokens via Supabase Auth | Verify your Acopio account for cloud export |
...
Acopio does **not** upload your full local library to the cloud automatically.
...
- **Supabase** — Authentication for Acopio sign-in
...
Acopio requests Chrome permissions needed for its features:
...
Acopio is not directed at children under 13.
...
For privacy questions about Acopio, open an issue at:
