...
- [ ] Right-click → "Collect this element with Harvest" opens the same tooltip.
...
Sanitization logic itself verified 2026-08-24 by injecting the actual `shared.js`+`sanitize.js` source into the live `xss-adversarial.html` page (via the Browser pane's `javascript_tool`, not the loaded extension — I don't have a working connection to real Chrome to click through the full extension) and running `Harvest.sanitizeCaptureElement` directly against each adversarial element, then checking the *actual attribute/DOM state* of the output (not string-matching, which gave two false alarms on the first pass — see note below):
- [x] `<script>` stripped from output; rendering the sanitized HTML back via `innerHTML` on a live page does **not** re-execute it (`window.__harvestXssFired` stays `false`) — this is the specific "never executes when viewed later" check Section 10 asks for.
...
- [ ] Manually inspect IndexedDB (`chrome-extension://<id>` → Application → IndexedDB → `harvest-db` → `items`) and confirm each captured item's shape matches Section 5.
...
- [x] Escape didn't close the tooltip on real (React/SPA) sites — the site's own `document`-level Escape handler ran first and swallowed the key before Harvest's listener ever saw it. Fixed by moving Harvest's keydown/click-outside listeners to `window` capture, which always fires before `document`-level listeners regardless of registration order. Also added click-outside-to-dismiss as a second, independent way to close it.
...
- [x] Oversized-component confirm used native `window.confirm()` — a blocking dialog that freezes the whole tab's main thread and looks like generic browser chrome, not Harvest's own design (explicitly against the spec's "shouldn't look like a generic dev-tools clone"). Replaced with an inline Cancel / "Capture anyway" state inside the tooltip card itself.
...
This is a deliberate departure from Section 2's "no capture-mode toggle" — added because always-on hover across every site was causing real confusion in testing ("how do I just browse normally"). Toggle lives in the side panel; state in `chrome.storage.local` (`harvestActive`, default `true`); toolbar icon shows an "OFF" badge when paused; content scripts subscribe to live storage changes so toggling takes effect on already-open tabs immediately, no page refresh needed. No new permission required (`storage` was already granted).
...
Small pill fixed to the bottom-right of every page: brand dot (drag handle), a toggle button (same `harvestActive` state as the side panel switch, kept in sync both directions), a button to open the side panel, and an X to hide the pill for the current page load (not persisted — reappears on refresh; a permanent per-site dismiss would need a new storage key, deferred until it's clear that's actually wanted). Real SVG icons (a generic cursor/select-tool glyph and a generic sidebar-panel glyph — standard UI iconography, not a copied brand mark) replaced the original unicode glyphs, which were reported as too vague to read. Shadow-DOM isolated like the tooltip, registered in the same `Harvest.ownRoots` list so hovering it never triggers the capture tooltip underneath. Draggable by the brand dot; position persists across reloads via `chrome.storage.local` (`harvestToolbarPos`), clamped to the viewport on restore so it can't end up off-screen.
...
- [x] Side panel's redundant "Harvest" text label removed from the in-panel topbar — Chrome's own native side-panel title bar already displays the full extension name, so the duplicate text added nothing.
- [x] Empty state (side panel, no items yet) given a small illustrated fan of blank type-badge cards instead of bare text — built from Harvest's own existing C/T/I badge iconography, not copied from the reference product's specific artwork/branding.
